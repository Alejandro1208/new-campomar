...
app.post('/api/products', async (req, res) => {
    try {
        ...
        const result = await pool.query('INSERT INTO products (name, description, category_id, image_url) VALUES ($1, $2, $3, $4) RETURNING *', [name, description, category_id, image_url]);
...
app.put('/api/products/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, category_id, image_url } = req.body;
        const result = await pool.query('UPDATE products SET name = $1, description = $2, category_id = $3, image_url = $4 WHERE id = $5 RETURNING *', [name, description, category_id, image_url, id]);
        res.json(result.rows[0]);
    }
    catch (error) {
...
app.post('/api/product-categories', async (req, res) => {
    try {
        const { name } = req.body;
        const result = await pool.query('INSERT INTO product_categories (name) VALUES ($1) RETURNING *', [name]);
        res.json(result.rows[0]);
    }
    catch (error) {
...
app.put('/api/social-media/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query('UPDATE social_media SET platform = $1, url = $2 WHERE id = $3 RETURNING *', [platform, url, id]);
        res.json(result.rows[0]);
    }
    catch (error) {
...
app.post('/api/phone-numbers', async (req, res) => {
    try {
        ...
        const result = await pool.query('INSERT INTO phone_numbers (number, description) VALUES ($1, $2) RETURNING *', [number, description]);
        res.json(result.rows[0]);
    }
    catch (error) {
...
app.put('/api/phone-numbers/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { number, description } = req.body;
        const result = await pool.query('UPDATE phone_numbers SET number = $1, description = $2 WHERE id = $3 RETURNING *', [number, description, id]);
        res.json(result.rows[0]);
    }
    catch (error) {
...
app.put('/api/business-hours/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query('UPDATE business_hours SET days = $1, hours = $2 WHERE id = $3 RETURNING *', [days, hours, id]);
        res.json(result.rows[0]);
    }
    catch (error) {
